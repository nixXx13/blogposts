...
class PullWorkerIterative extends Worker{
    taskIdList
    getCurrentTaskIndex
    data
    ...
    async work() {
        let currentTaskIndex = this.getCurrentTaskIndex();
        let currentTaskId = this.taskIdList[currentTaskIndex];
        while( currentTaskId ) {
            ...
            currentTaskIndex = this.getCurrentTaskIndex();
            currentTaskId = this.taskIdList[currentTaskIndex];
        }
    }
}
...
async function main() {
    ...
    const workers = [
        new PullWorkerIterative(taskIds, getCurrentTaskIndex, data),
        new PullWorkerIterative(taskIds, getCurrentTaskIndex, data),
    ]
